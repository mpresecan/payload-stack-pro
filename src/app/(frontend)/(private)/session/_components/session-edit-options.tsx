...
import { MoreVertical, Edit, X, Trash, RefreshCw, UserMinus } from 'lucide-react'
...
import { removeMeAsPresenter } from '../_actions/remove-me-as-presenter'

interface SessionOptionsProps {
  session: SessionEvent
  allowCancel?: boolean,
  setRemovePresenter?: boolean
}

export default function SessionEditOptions({
                                             session,
                                             allowCancel = true,
                                             setRemovePresenter = false,
                                           }: SessionOptionsProps) {
  ...
  const handleRemoveConfirm = async () => {
    try{
      await removeMeAsPresenter(session.id)
      toast.success('Removed as presenter')
      router.refresh()
    } catch (error) {
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem asChild>
            <Link href={`/${!allowCancel ? 'suggested-topic' : 'session'}/${session.id}/edit`}
          ...
          {setRemovePresenter &&
            <DropdownMenuItem onSelect={() => setShowRemoveDialog(true)}>
              ...
              <span>Remove me as presenter</span>
            </DropdownMenuItem>
          }
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you sure you want to delete
              this {!allowCancel ? 'topic' : 'session'}?</AlertDialogTitle>
            <AlertDialogDescription>
              {allowCancel ? (
                isCanceled ? (
        ...
        <AlertDialog open={isCancelAlertOpen} onOpenChange={setIsCancelAlertOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                When a session is canceled, people can still vote and show their interest. You can uncanel the session
                later if needed.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
      ...
      {setRemovePresenter &&
        <AlertDialog open={showRemoveDialog} onOpenChange={setShowRemoveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Remove as Presenter</AlertDialogTitle>
              <AlertDialogDescription>
                Are you sure you want to remove yourself as a presenter from this session?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancel</AlertDialogCancel>
