...
const SessionTabs = () => {
  const { tab, setTab, queryPastSessions } = useSessionFilter()
    ...
    <Tabs value={tab} onValueChange={setTab} className="mb-6">
      <ScrollArea className="w-full whitespace-nowrap pb-1.5">
        <TabsList className="w-full flex justify-between">
          ...
          <TabsTrigger value="proposals" disabled={queryPastSessions} className='w-1/2 md:w-1/5'>Proposals</TabsTrigger>
          <TabsTrigger value="scheduled" disabled={queryPastSessions} className='w-1/2 md:w-1/5'>Scheduled</TabsTrigger>
